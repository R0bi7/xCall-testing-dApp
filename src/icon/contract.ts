...
export class Contract {
  ...
  /**
   *
   * @param sig - Event signature (e.g. 'CallMessage(str,str,int,int)')
   * @param startBlock (Optional) Start block from which to start searching for event (default = latest)
   */
  async waitEvent(
      sig: string,
      startBlock?: number
  ): Promise<{ events: EventLog[], height: number}> {
    let latest = await this.getBlock(startBlock ? startBlock : "latest");
